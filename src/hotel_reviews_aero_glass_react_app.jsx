 ...
 *
 * Notas técnicas:
 * - Tailwind + UI mínima + lucide-react + Framer Motion
...
const Tag = ({ children }) => (
  <Badge className="bg-gradient-to-r from-emerald-500/80 via-sky-500/80 to-amber-600/80 text-white border-0 shadow">
    {children}
  </Badge>
);
...
export default function HotelReviewsAeroGlass() {
  ...
  const filtered = useMemo(() => {
    ...
    return HOTELS.filter((h) => [h.name, h.location, ...(h.tags || [])].join(" ").toLowerCase().includes(q));
  }, [query]);
  ...
  const askAI = async () => {
    ...
    const mock = {
      ...
      a:
        hint ||
        "Según tus preferencias, ambos hoteles son buenas opciones. Filtra por ‘Eco’ o ‘Frente al mar’ para acotar.",
    };
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-900 text-slate-900">
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-6">
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-emerald-200/40 via-sky-200/40 to-amber-200/40 border-b border-white/30">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Button variant="secondary" className="bg-white/50 hover:bg-white/70 border-white/40">
                Iniciar sesión
              </Button>
            ...
            <div className="grid sm:grid-cols-5 gap-3">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="outline" className="rounded-2xl bg-white/50 border-white/40">
                    Filtros
                  </Button>
                </DialogTrigger>
                <DialogContent className="backdrop-blur-2xl bg-white/70 border-white/50">
                  ...
                  <div className="grid sm:grid-cols-3 gap-3">
                    {["Eco", "Frente al mar", "Spa", "Centro", "Pet-friendly", "Cafetería"].map((f) => (
                      <Button key={f} variant="secondary" className="bg-white/70 border-white/50 rounded-xl">
                        {f}
                      </Button>
                    ))}
                  </div>
            ...
            <ScrollArea className="h-[72vh] lg:col-span-1 pr-2">
              <div className="grid gap-4">
                {filtered.map((h) => (
                  <Card
                    key={h.id}
                    className={`overflow-hidden border-white/40 bg-white/30 backdrop-blur-xl hover:bg-white/40 transition ${
                      activeHotel.id === h.id ? "ring-2 ring-emerald-400/60" : ""
                    }`}
                  >
                    <div className="flex">
                      ...
                      <div className="flex-1 p-3">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          <div className="text-sm opacity-80">
                            Desde <b>€{h.priceFrom}</b>/noche
                          </div>
                          <Button
                            size="sm"
                            className="rounded-xl bg-gradient-to-r from-emerald-600 to-sky-700 border-0"
              ...
              <Card className="border-white/40 bg-white/30 backdrop-blur-xl">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm opacity-80 flex items-center gap-2 flex-wrap">
                        ...
                        <span className="inline-flex items-center gap-1">
                          <Clock className="w-4 h-4" /> Check‑in 15:00 • Check‑out 12:00
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <CardContent>
                  <Tabs defaultValue="servicios" className="w-full">
                    <TabsList className="bg-white/50 border border-white/50 flex flex-wrap gap-2">
                    ...
                    <TabsContent value="servicios" className="mt-4">
                      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {activeHotel.services.map((s, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-3 rounded-2xl border border-white/40 bg-white/40 px-3 py-2 backdrop-blur-xl"
                          >
                      ...
                      {activeHotel.rooms.map((r, i) => (
                        <div key={i} className="rounded-2xl border border-white/40 bg-white/40 backdrop-blur-xl p-4">
                          ...
                          <div className="mt-3 flex gap-2">
                            <Button className="rounded-xl bg-gradient-to-r from-emerald-600 to-sky-700 border-0">
                              Reservar
                            </Button>
                            <Button variant="outline" className="rounded-xl bg-white/60 border-white/50">
                              Ver detalles
                            </Button>
                          </div>
                        </div>
                    ...
                    <TabsContent value="menu" className="mt-4 grid sm:grid-cols-2 gap-3">
                      {activeHotel.menu.map((m, i) => (
                        <div
                          key={i}
                          className="rounded-2xl border border-white/40 bg-white/40 backdrop-blur-xl p-4 flex items-center justify-between"
                        >
                          <div className="flex items-center gap-2">
                            <Utensils className="w-5 h-5" />
                            <span>{m.name}</span>
                            {m.tag && <Badge className="ml-2 bg-amber-600 border-0 text-white">{m.tag}</Badge>}
                          </div>
                    ...
                    <TabsContent value="fotos" className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {activeHotel.photos.map((src, i) => (
                        <img
                          ...
                          src={src}
                          alt={`Foto ${i + 1}`}
                          className="w-full h-44 object-cover rounded-2xl border border-white/40 shadow"
                          loading="lazy"
                        />
                      ))}
                    </TabsContent>
                      ...
                      {activeHotel.nearby.food.map((r, i) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <Badge className="bg-white/60 text-black border-white/40">{r.distance}</Badge>
                          </div>
                    ...
                    <TabsContent value="comollegar" className="mt-4 grid lg:grid-cols-2 gap-4">
                      <div className="space-y-3">
                        {activeHotel.nearby.transport.howTo.map((h, i) => (
                          <div
                            key={i}
                            className="rounded-2xl border border-white/40 bg-white/40 backdrop-blur-xl p-3 flex items-center gap-3"
                          >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-sm opacity-80">
                              Mínimo {activeHotel.nearby.longStay.minNights} noches • Desde <b>€{activeHotel.nearby.longStay.monthlyFrom}</b>/mes
                            </p>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {activeHotel.nearby.longStay.perks.map((p) => (
                              <Tag key={p}>{p}</Tag>
                            ))}
                          </div>
                        </div>
                ...
                <CardContent>
                  <div className="flex items-start gap-2">
                    <Textarea
                      ...
                      placeholder="Ej.: ¿Qué hotel tiene mejor spa y opción mensual?"
                      className="bg-white/60 border-white/50"
                    />
                    <Button
                      onClick={askAI}
                      className="h-10 mt-0.5 rounded-xl bg-gradient-to-r from-emerald-600 to-sky-700 border-0 whitespace-nowrap"
                    >
                      Preguntar
                    </Button>
                  </div>
                    ...
                    {aiResponses.map((r) => (
                      <div key={r.id} className="rounded-2xl border border-white/40 bg-white/50 backdrop-blur-xl p-3">
                        ...
                        <div className="text-sm leading-relaxed">
                          <span className="font-semibold">IA:</span> {r.a}
                        </div>
                      </div>
                    ))}
                    {!aiResponses.length && (
                      <div className="text-sm opacity-70">
                        Escribe una consulta y presiona <em>Preguntar</em>. Integra tu API/LLM en <code>askAI()</code>.
                      </div>
                    )}
                  </div>
                </CardContent>
        ...
        {/* Pie */}
        <div className="text-center text-xs opacity-70 mt-4 text-white">
          <span className="inline-flex items-center gap-1">
            Hecho con estilo <em>AeroGlass</em> <PenSquare className="w-3 h-3" /> • paleta verde/azul/café
          </span>
        </div>
      </div>
    </div>
